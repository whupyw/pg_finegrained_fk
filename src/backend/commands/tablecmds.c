...
static void addFkArrayConstraint(Oid conoid, int numarrayfks,
								 int16 *fkarrayattnum, int16 *fkarraynum);
static int transformArrayColumnList(Oid relId, List *colList,
							   		int16 *fkarrayattnum, int16* fkarraynum);
...
static void ATExecDropConstraint(Relation rel, const char *constrName,
								 ...
								 bool recurse, bool recursing,
								 bool missing_ok, LOCKMODE lockmode);
static ObjectAddress ATExecModifyConstraint(Relation rel, const char *constrName,
...
AlterTableGetLockLevel(List *cmds)
{
	...
	foreach(lcmd, cmds)
	{
		...
		switch (cmd->subtype)
		{
			...
			case AT_ModifyConstraint: /* as MODIFY INDEX */
		  ...
		  AlterTableUtilityContext *context)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_DropConstraint: /* DROP CONSTRAINT */
			...
			pass = AT_PASS_DROP;
			break;
		case AT_ModifyConstraint: /* MODIFY CONSTRAINT */
			ATSimplePermissions(cmd->subtype, rel, ATT_TABLE | ATT_FOREIGN_TABLE);
			ATCheckPartitionsNotInUse(rel, lockmode);
			/* Other recursion occurs during execution phase */
			/* No command-specific prep needed except saving recurse flag */
		  ...
		  AlterTableUtilityContext *context)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_DropConstraint: /* DROP CONSTRAINT */
			...
			break;
		case AT_ModifyConstraint: /* MODIFY CONSTRAINT */
			address = ATExecModifyConstraint(rel, cmd->name, cmd->mbehavior,
											 cmd->recurse, false,
											 cmd->missing_ok, lockmode);
			break;
		case AT_AlterColumnType:	/* ALTER COLUMN TYPE */
...
ChooseForeignKeyConstraintNameAddition(List *colnames)
{
	...
	foreach(lc, colnames)
	{
		...
		const Node* n = (const Node*)lfirst(lc);
		if (n->type == T_String)
			name = strVal(lfirst(lc));
		else if (n->type == T_ArraySubfield) {
			const ArraySubfield* Subfield = (const ArraySubfield*)lfirst(lc);
			name = Subfield->colname;
			int index = Subfield->subfield;
			name = psprintf("%s_%d", name, index);
		}
						  ...
						  bool recurse, bool recursing, LOCKMODE lockmode)
{
	...
	int16		fkarrayattnum[INDEX_MAX_KEYS] = {0};
	int16		fkarraynum[INDEX_MAX_KEYS] = {0};
	...
	numarrayfks = transformArrayColumnList(RelationGetRelid(rel), 
									 fkconstraint->fk_attrs,
									 fkarrayattnum, fkarraynum);
	...
	/* 添加数组子级约束 */
	if (numarrayfks > 0) 
		addFkArrayConstraint(address.objectId, 
							 numarrayfks, 
							 fkarrayattnum, 
							 fkarraynum);
...
static void 
addFkArrayConstraint(Oid conoid, 
					 int numarrayfks,
					 int16 *fkarrayattnum, 
					 int16 *fkarraynum)
{
	int i;
	for (i = 0; i < numarrayfks; i++)
	{
		Oid oid = CreateConKeyPathEntry(conoid, 
							  fkarrayattnum[i], 
							  fkarraynum[i]);
	}
}
...
static int 
transformArrayColumnList(Oid relId, List *colList,
						int16 *fkarrayattnum, 
						int16* fkarraynum)
{
	ListCell *lc;
	...
	{
		const Node* n = (const Node*)(lfirst(lc));
		if (n->type == T_ArraySubfield) {
			const ArraySubfield* Subfield = (const ArraySubfield*)lfirst(lc);
			HeapTuple tup;
			Form_pg_attribute attform;

			tup = SearchSysCacheAttName(relId, Subfield->colname);
			attform = (Form_pg_attribute) GETSTRUCT(tup);

			fkarraynum[i] = Subfield->subfield;
			fkarrayattnum[i] = attform->attnum;
			i++;
			ReleaseSysCache(tup);
		}
	}
						...
						int16 *attnums, Oid *atttypids)
{
	...
	foreach(l, colList)
	{
		const Node* n = (const Node*)(lfirst(l));
		char	   *attname;
		if (n->type == T_String) 
			attname = strVal(lfirst(l));
		else if (n->type == T_ArraySubfield) {
			const ArraySubfield* Subfield = (const ArraySubfield*)lfirst(l);
			attname = Subfield->colname;
		}		
		HeapTuple	atttuple;
		Form_pg_attribute attform;
		...
		if (atttypids != NULL) {
			if (attform->atttypid == INT4ARRAYOID && n->type == T_ArraySubfield)
				atttypids[attnum] = INT4OID;
			else atttypids[attnum] = attform->atttypid;
		}
		ReleaseSysCache(atttuple);
		attnum++;
	}
...
/*
 * ATExecModifyConstraint
 *      Modify the state of a constraint (enable/disable)
 *
 * rel: relation containing the constraint
 * constrName: name of the constraint to modify
 * behavior: enable/disable behavior (ENABLE/DISABLE)
 * recurse: whether to recurse to child tables
 * recursing: true if this is a recursive call
 * missing_ok: if true, don't error if constraint doesn't exist
 * lockmode: lock mode to use
 */
static ObjectAddress
ATExecModifyConstraint(Relation rel, const char *constrName,
                       ModifyBehavior behavior,
                       bool recurse, bool recursing,
                       bool missing_ok, LOCKMODE lockmode)
{
    ...
    /* Determine the new state based on the behavior */
    switch (behavior)
    {
        case CONSTR_WITH_CHECK:
            newstate = false;
            break;
        case CONSTR_NO_CHECK:
            newstate = true;
            break;
        default:
            elog(ERROR, "unrecognized constraint modification behavior: %d",
                 (int) behavior);
            newstate = false;  /* keep compiler quiet */
    }

    /* Look up the constraint */
    constraintOid = get_relation_constraint_oid(RelationGetRelid(rel),
    ...
    /* Open pg_constraint and fetch the tuple */
    conrel = table_open(ConstraintRelationId, RowExclusiveLock);
    contup = SearchSysCacheCopy1(CONSTROID, ObjectIdGetDatum(constraintOid));
    if (!HeapTupleIsValid(contup))
        elog(ERROR, "cache lookup failed for constraint %u", constraintOid);
    con = (Form_pg_constraint) GETSTRUCT(contup);

    /* Check if we're actually changing the state */
    if (con->conisvalid == newstate)
    {
        /* No change needed */
        table_close(conrel, RowExclusiveLock);
    ...
    /* Check permission - must own table or be superuser */
    // if (!pg_class_ownercheck(RelationGetRelid(rel), GetUserId()))
    //     aclcheck_error(ACLCHECK_NOT_OWNER, get_relkind_objtype(rel->rd_rel->relkind),
    //                    RelationGetRelationName(rel));

    /* For CHECK constraints, we can just update the pg_constraint entry */
    if (con->contype == CONSTRAINT_FOREIGN)
    {
        /* For foreign key constraints, we need to enable/disable the triggers */
        Oid         conrelid = con->conrelid;
        Oid         confrelid = con->confrelid;
        Oid         delTrigOid;
        Oid         updTrigOid;

        /* Update the pg_constraint entry first */
        con->conisvalid = newstate;
        CatalogTupleUpdate(conrel, &contup->t_self, contup);
    }

    /* If requested, recurse to child tables */
    if (recurse && !recursing)
    {
        child_rels = find_all_inheritors(RelationGetRelid(rel), lockmode, NULL);
        
        foreach(lc, child_rels)
        {
            ...
            /* Recursive call */
            ATExecModifyConstraint(childrel, constrName, behavior,
                                   true, true, missing_ok, lockmode);
    ...
    /* Return the address of the modified constraint */
    ObjectAddressSet(address, ConstraintRelationId, constraintOid);
    return address;
}
