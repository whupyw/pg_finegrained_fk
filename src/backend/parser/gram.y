...
%type <mbehavior> 	opt_modify_behavior

%type <node>	array_subfield_elem
%type <list>	complex_subfield_list
...
%token <keyword> ABORT_P ABSENT ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	VACUUM VALID VALIDATE VALIDATION VALIDATOR VALUE_P VALUES VARCHAR VARIADIC VARYING
...
opt_modify_behavior:
			WITH VALIDATION					{ $$ = CONSTR_WITH_CHECK; }	
			| NO VALIDATION					{ $$ = CONSTR_NO_CHECK; }
			| /* EMPTY */					{ $$ = CONSTR_WITH_CHECK; /* default */ }
		;
...
alter_table_cmd:
			...
			/* ALTER TABLE <name> MODIFY CONSTRAINT ... IF NOT EXISTS */
			| MODIFY CONSTRAINT IF_P EXISTS name opt_modify_behavior
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);

					n->subtype = AT_ModifyConstraint;
					n->name = $5;
					n->mbehavior = $6;
					n->missing_ok = true;
					$$ = (Node *) n;
				}
			/* ALTER TABLE <name> MODIFY CONSTRAINT ...  */
			| MODIFY CONSTRAINT name opt_modify_behavior
				{
					AlterTableCmd *n = makeNode(AlterTableCmd);
...
ConstraintElem:
			...
			| FOREIGN KEY '(' complex_subfield_list ')' REFERENCES qualified_name
				opt_column_list key_match key_actions ConstraintAttributeSpec
				{
					Constraint *n = makeNode(Constraint);
...
array_subfield_elem: ColId '[' Iconst ']'
			{
				ArraySubfield *n = makeNode(ArraySubfield);
				n->colname = $1;
				n->subfield = $3;
				n->location = @1;
				$$ = (Node *) n;
...
complex_subfield_list:
			array_subfield_elem										{ $$ = list_make1($1); }
			| complex_subfield_list ',' array_subfield_elem			{ $$ = lappend($1, $3); }
		;
