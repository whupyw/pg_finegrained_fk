...
typedef struct RI_ArrayConstraintInfo
{
	Oid			constraint_id;	/* OID of pg_constraint entry (hash key) */
	int16		fk_attnum; /* attnums of referencing cols */
	int16		fk_array_attnum; /* attnums of referencing cols */
} RI_ArrayConstraintInfo;
...
static const RI_ArrayConstraintInfo *ri_FetchArrayConstraintInfo(Oid constraintOid, int16 conkey);
static const RI_ConstraintInfo *ri_LoadConstraintInfo(Oid constraintOid);
static const RI_ArrayConstraintInfo *ri_LoadArrayConstraintInfo(Oid constraintOid, int16 conkey);
...
RI_FKey_check(TriggerData *trigdata)
{
	...
	if ((qplan = ri_FetchPreparedPlan(&qkey)) == NULL)
	{
		...
		for (int i = 0; i < riinfo->nkeys; i++)
		{
			...
			RI_ArrayConstraintInfo* arrayinfo = ri_FetchArrayConstraintInfo(riinfo->constraint_id, 
																			riinfo->fk_attnums[i]);
			...
			if (arrayinfo != NULL)
			{
				/* 如果是数组外键，则给paramname添加数组下标访问 */
				char fk_array_access[NAMEDATALEN * 2];
				snprintf(fk_array_access, sizeof(fk_array_access), 
						"(%s)[%d]", paramname, arrayinfo->fk_array_attnum);
				ri_GenerateQual(&querybuf, querysep,
							attname, pk_type,
							riinfo->pf_eq_oprs[i],
							fk_array_access, fk_type);
			}
			else
			{
				/* 普通外键处理 */
	...
	if ((qplan = ri_FetchPreparedPlan(&qkey)) == NULL)
	{
		...
		for (int i = 0; i < riinfo->nkeys; i++)
		{
			...
			if (arrayinfo != NULL)
			{
				...
				char fk_array_access[NAMEDATALEN * 2];
				snprintf(fk_array_access, sizeof(fk_array_access), 
						"(%s)[%d]", attname, arrayinfo->fk_array_attnum);
...
RI_Initial_Check(Trigger *trigger, Relation fk_rel, Relation pk_rel)
{
	...
	for (int i = 0; i < riinfo->nkeys; i++)
	{
		...
		if (arrayinfo != NULL)
		{
			// 数组外键处理
			char fk_array_access[NAMEDATALEN*2];
			snprintf(fk_array_access, sizeof(fk_array_access), "(%s)[%d]", fkattname, arrayinfo->fk_array_attnum);
	...
	for (int i = 0; i < riinfo->nkeys; i++)
	{
		...
		if (arrayinfo != NULL)
		{
			/* 数组元素不为空检查 */
			appendStringInfo(&querybuf,
						"%s(fk.%s)[%d] IS NOT NULL",
						sep, fkattname, arrayinfo->fk_array_attnum);
		}
		else
		{
			/* 普通列不为空检查 */
...
/*
 * Fetch the RI_ArrayConstraintInfo struct for the trigger's FK constraint.
 */
static const RI_ArrayConstraintInfo*
ri_FetchArrayConstraintInfo(Oid constraintOid, int16 conkey)
{
	const RI_ArrayConstraintInfo *ri_arrayinfo;

	/* Find or create a hashtable entry for the constraint */
	ri_arrayinfo = ri_LoadArrayConstraintInfo(constraintOid, conkey);

	return ri_arrayinfo;
}

static const RI_ArrayConstraintInfo*
ri_LoadArrayConstraintInfo(Oid constraintOid, int16 conkey)
{
	RI_ArrayConstraintInfo *ri_arrayinfo = palloc0(sizeof(RI_ArrayConstraintInfo)); 
	bool		found;
	HeapTuple	tup;
	Form_pg_conkeypath conkeypathForm;

	tup = SearchSysCache2(CONKEYPATHOID, ObjectIdGetDatum(constraintOid), Int16GetDatum(conkey));
	
	if (!HeapTupleIsValid(tup)) /* should not happen */
		return NULL;
	conkeypathForm = (Form_pg_conkeypath) GETSTRUCT(tup);

	ri_arrayinfo->constraint_id = conkeypathForm->conoid;
	ri_arrayinfo->fk_attnum = conkeypathForm->conkey;
	ri_arrayinfo->fk_array_attnum = conkeypathForm->conkeypath;
...
ri_LoadConstraintInfo(Oid constraintOid)
{
	...
	riinfo->conisValid = conForm->conisvalid;
...
ri_HashCompareOp(Oid eq_opr, Oid typeid)
{
	 ...
	 */
	key.eq_opr = eq_opr;
	if (eq_opr == Int4EqualOperator && typeid == INT4ARRAYOID) 
		key.eq_opr = ARRAY_EQ_OP;
	key.typeid = typeid;
	...
	if (!entry->valid)
	{
		...
		fmgr_info_cxt(get_opcode(key.eq_opr), &entry->eq_opr_finfo,
					  TopMemoryContext);
		 ...
		 */
		op_input_types(key.eq_opr, &lefttype, &righttype);
